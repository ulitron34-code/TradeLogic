...
function applyProductionMigrations() {
  ...
  // Previous Render releases recorded migrations 7, 9, 11 and 12 as failed. Resolve
  // only those known markers so Prisma can retry the corrected migrations.
  for (const migration of ['7_add_postgres_classification_queue', '9_add_case_assignments', '11_add_new_table_rls', '12_add_case_review_requests']) {
    try {
