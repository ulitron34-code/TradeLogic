...
describe('jurisprudence vector search', () => {
  it('passes a pgvector literal and returns ranked matches', async () => {
    ...
    expect(calls[0]).toMatchObject({ values: ['[0.1,0.2]', '[0.1,0.2]', 0.65, '[0.1,0.2]', 5] });
  });

  it('rejects malformed embeddings and bounds the result window', async () => {
    await expect(findSimilarPrecedents([Number.NaN], 100, { $queryRaw: async () => [] } as any)).rejects.toThrow('finite');
  });
});
