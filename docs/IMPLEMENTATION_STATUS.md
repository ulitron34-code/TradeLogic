...
- Render responde en `https://tradelogic-api.onrender.com`: `/health`, `/ready` y `/version` devuelven 200; `/ready` confirma `database: ok`, pero todavia no incluye `redis: ok`. La verificacion del 2026-08-12 encontro que produccion sigue sirviendo `7ed9f62aebd6b3ebe52c5e97694ca661fa6c16e5`, mientras GitHub ya tenia `79319b5d2e638a5c7d0c090d556a61ea16bdcc6d`; el deploy `dep-d9u9hrfmnsvc73a8i880` fue iniciado para `79319b5` pero aun no se ha promovido. Los logs rojos `getaddrinfo ENOTFOUND red...` apuntan a `REDIS_URL` incompleto o incorrecto en Render; ver `docs/DEPLOYMENT_RUNBOOK.md`.
