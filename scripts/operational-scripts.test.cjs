...
test('diagnose-deployment explains healthy API with missing version route', async (t) => {
  ...
  assert.equal(summary.expectedRenderWebSettings.startCommand, 'pnpm --filter @platform/db prisma:deploy && node apps/api/dist/server.js');
...
test('includes optional deployment diagnosis in pilot readiness output', () => {
  ...
  writeFileSync(diagnosisPath, `${JSON.stringify({
    ...
    renderRecovery: {
      ...
      expectedSettings: { startCommand: 'pnpm --filter @platform/db prisma:deploy && node apps/api/dist/server.js', healthCheckPath: '/health' },
  ...
  assert.equal(readiness.diagnostics[0].details.renderRecovery.expectedSettings.startCommand, 'pnpm --filter @platform/db prisma:deploy && node apps/api/dist/server.js');
...
test('prepare-pilot-evidence summary inherits pilot readiness next action', () => {
  ...
  assert.equal(summary.next, 'Settings -> Build & Deploy: confirmar Start Command = pnpm --filter @platform/db prisma:deploy && node apps/api/dist/server.js; despues ejecutar Manual Deploy -> Deploy latest commit.');
});
